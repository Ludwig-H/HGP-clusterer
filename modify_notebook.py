...
if X_clustering is not None and len(X_clustering) > 0:
    ...
    for i in range(NUM_FRAMES):
        ...
        # Offset +1 car l'ID 0 est souvent réservé au background
    ...
    !python /content/semantic-kitti-api/evaluate_panoptic.py --dataset {eval_dir} --predictions {pred_dir} --split valid --data_cfg /content/semantic-kitti-api/config/semantic-kitti.yaml
else:
...
for cell in nb["cells"]:
    if cell["cell_type"] == "code":
        if cell["source"] and "5.1 Évaluation LSTQ (Panoptic)" in cell["source"][0]:
