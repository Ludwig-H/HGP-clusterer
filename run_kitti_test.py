...
if DOWNLOAD_DATA:
    ...
    # Destination racine
    base_dest = "/workspaces/HGP-clusterer/semantic_kitti_test"
...
try:
    loader = SemanticKITTILoader("/workspaces/HGP-clusterer/semantic_kitti_test", SEQUENCE_TO_TEST)
...
def compute_uot_matrix(tracks, cloud_pts, prior):
    ...
    b_f = torch.full((K_total,), float(N), device=DEVICE) # Capacité relâchée pour que la piste puisse absorber tout un cluster
...
for t in frames:
    ...
    for sem_cl in classes_in_frame:
        ...
        confirmed_tracks = [tr for tr in tracks_dict.values() if tr.state == "Confirmed" and tr.semantic_class == sem_cl]
